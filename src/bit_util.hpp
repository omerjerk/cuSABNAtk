...
inline int set_size(uint64_t S) { return __builtin_popcountl(S); }

inline int set_size(uint_type<1> S) { return __builtin_popcountl(S.b[0]); }

template <int N> inline int set_size(const uint_type<N>& S) {
    int w = 0;
    for (int i = 0; i < N; ++i) w += __builtin_popcountl(S.b[i]);
    return w;
} // set_size
...
template <typename Iter, typename set_type> inline set_type as_set(Iter first, Iter last) {
    set_type S = 0;
